...
namespace Common
{
    public class UploadPic
    {
        public static string Upload(FileUpload Object, string PicSize, string DelFileName)
        {
            ...
            if (Object.HasFile)
            {
                ...
                if (fileContentType == "image/bmp" || fileContentType == "image/gif" || fileContentType == "image/pjpeg")
                {
                    ...
                    //上传路径设置
                    string uploadPath = string.Format(@"{0}\{1}\{2}\", StringPlus.GetPhysicalPath(), ConfigurationManager.AppSettings["UploadPath"], PicSize);
                    //按年月生成文件夹
                    string folderName = StringPlus.MakeFolderName();
                    //生成新文件名
                    string fileName = StringPlus.MakeFileRndName() + file.Extension.ToLower();
                    //服务器端文件路径
        ...
        {
            originalImagePath = string.Format(@"{0}{1}\{2}", StringPlus.GetPhysicalPath(), ConfigurationManager.AppSettings["UploadPath"], originalImagePath);
            thumbnailPath = string.Format(@"{0}{1}\{2}", StringPlus.GetPhysicalPath(), ConfigurationManager.AppSettings["UploadPath"], thumbnailPath);
        ...
        {
            string FilePath = string.Format(@"{0}\{1}\{2}", StringPlus.GetPhysicalPath(), ConfigurationManager.AppSettings["UploadPath"], PicPath);
