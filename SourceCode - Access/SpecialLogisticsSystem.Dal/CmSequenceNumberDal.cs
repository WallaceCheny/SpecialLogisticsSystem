...
namespace SpecialLogisticsSystem.Dal
{
    public class CmSequenceNumberDal : ICmSequenceNumberDal
    {
        ...
        public string GetNumber(CodeTable table)
        {
            ...
            T_Cm_SequenceNumber item = GetList(queryInfo).FirstOrDefault();
            item.max_index = item.max_index + 1;
            Update(item);
            ////queryInfo.SetXml("GetNumber");
            //IList<dynamic> codeList = _dao.Select(queryInfo);
            //foreach (dynamic model in codeList)
            //{
            strCode = item.prefix + DateTime.Now.ToString(item.date_type) + item.infix 
                + string.Format("{0:D" + item.index_length +"}", item.max_index) + item.suffix;
            //}
            return strCode;
        }
    }
